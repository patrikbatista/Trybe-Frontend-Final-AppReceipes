...
describe("2 - Rotas", () => {
  ...
  describe('Ao entrar na rota "/explorar/comidas"', () => {
    it("E clicar em me surpreenda, uma receita aleatória deve ser chamada", async () => {
      global.fetch = jest.fn().mockImplementation(() => foodResponse);
      await act(async () => {
...
describe("3 - Tela de comidas e bebidas", () => {
  describe("Implemente o botão All", () => {
  ...
  describe("Implemente o botão das outras categorias", () => {
  ...
  describe("Todos os links do footer estão na tela", () => {
    it(" ao clicar no ícone de comidas redireciona para pagina de bebidas", async () => {
      await act(async () => {
        renderWithRouterAndRedux(<Foods />);
  ...
  describe("Teste se o componente searchBar funciona", () => {
    it("Ao clicar no botão search a barra de pesquisa aparece", () => {
      const { getByTestId } = renderWithRouterAndRedux(<Foods />);
      fireEvent.click(getByTestId("search-top-btn"));
      expect(getByTestId("search-input")).toBeInTheDocument();
    });

    it("É possível digitar algo na barra de pesquisa", () => {
      const { getByTestId } = renderWithRouterAndRedux(<Foods />);
      fireEvent.click(getByTestId("search-top-btn"));
...
describe("4 - Tela de detalhes das receitas", () => {
  ...
  describe("Botão de favorito", () => {
    it("Deve existir um botão de favoritar", async () => {
      await act(async () => {
        renderWithRouterAndRedux(<FoodDetails />);
      });
